...
const RegistrationOverview = (): JSX.Element => {
  const { firstName, lastName, age, phoneNumber, emailAddress, seatNr, foodPreference, allergyInfo, sectorId, agreeToTerms } = useAppSelector(
    (state) => state.user,
  );
    ...
    <OverviewStyle>
      ...
      <div className="row personal">
        <p>
          ...
          <span data-testid="firstName" className="value">
            {firstName}
          </span>
        </p>
        <p>
          <span className="label">Last Name: </span>{' '}
          <span data-testid="lastName" className="value">
            {lastName}
          </span>
        </p>
        <p>
          <span className="label">Age: </span>{' '}
          <span data-testid="age" className="value">
            {age}
          </span>
        </p>
        <p>
          <span className="label">Sector: </span> <span className="value">{userCurrentSector}</span>
        </p>
      </div>
      <div className="row contact">
        <p>
          <span className="label">Phone number: </span>{' '}
          <span data-testid="phone" className="value">
            {phoneNumber}
          </span>
        </p>
        <p>
          <span className="label">Email address: </span>{' '}
          <span data-testid="email" className="value">
            {emailAddress}
          </span>
        </p>
      </div>
      ...
      <div className="row misc">
        <p>
          ...
          <span data-testid="seat" className="value">
            {seatNr}
          </span>
        </p>
        <p>
          <span className="label">food preference: </span>{' '}
          <span data-testid="food" className="value">
            {foodPreference}
          </span>
        </p>
        <p>
          <span className="label">allergie info: </span>{' '}
          <span data-testid="allergies" className="value">
            {allergyInfo}
          </span>
        </p>
      </div>
      <div className="buttonGrp">
        <FormButton type="button" testId="back" onClick={() => dispatch(setCurrentStep(currentStepIndex - 1))} value="back" />
